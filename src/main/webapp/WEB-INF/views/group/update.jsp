...
<body>
    ...
    <form action="/group/update" method="post">
        ...
        <input type="number" id="maxUserNum" name="maxUserNum" value="${group.maxUserNum}" min="4" max="99" required />
        <br><br>
...
<script>
  ...
  // 기존 location 값에서 시/군 분리 및 기본 선택
    window.addEventListener('load', function () {
      const location = "${group.location}";
      const [city, country] = location.split(" ");
      citySelect.value = city;
      citySelect.dispatchEvent(new Event('change'));  // 군/구 select 초기화
      setTimeout(() => {
        countrySelect.value = country;
      }, 100);  // select가 채워지고 나서 값 세팅
    });
</script>
