...
<body>
    ...
    <form action="/group/update" method="post">
        ...
        <input type="number" id="maxUserNum" name="maxUserNum" value="${group.maxUserNum}" min="${group.maxUserNum}" max="99" required />
        <br><br>

        <button type="submit">수정 완료</button>
        <a href="/group/detail?groupId=${group.id}">취소</a>
    ...
    <script>
      ...
      // city 선택 시 구/군 채우는 함수
        ...
        function bestCityKey(raw) {
          ...
          // 나머지 문자열에서 regionData[cityKey]에 "존재하는" 항목을 선택
          function pickCountry(cityKey, rest) {
            ...
            const tokens = cleaned.split(' '); // 예: ["수원시","장안구"]
            const candidates = [];
          ...
          document.addEventListener('DOMContentLoaded', function () {
            const raw = '<c:out value="${group.location}" />'.trim(); // 안전하게 출력
            if (!raw) return;

            // 1) 시/도 키 고르기 (가장 긴 prefix)
            const cityKey = bestCityKey(raw);
            if (!cityKey) {
              console.log('[debug] 시/도 키 매칭 실패:', raw);
              return;
            }

            // 2) 구/군(또는 시) 후보 고르기
            const rest = raw.slice(cityKey.length).trim(); // 시/도 뺀 나머지
            ...
            if (country) {
              ...
              console.log('[debug] 구/군 매칭 실패:', { cityKey, rest });
            }
          });
