    ...
    <form action="/group/create" method="post" enctype="multipart/form-data" class="needs-validation" novalidate>
      ...
      <div class="mb-3">
        <div class="label-row">
          ...
          <span class="char-counter" id="titleCounter">
        ...
        <input type="text"
               ...
               maxlength="100"
               data-max="100">
        <div class="invalid-feedback">모임 이름을 입력해주세요. (최대 100자)</div>
      </div>
      ...
      <div class="mb-3">
        ...
        <div class="invalid-feedback">대표 사진을 추가해주세요.</div>
      </div>
      ...
      <div class="mb-3">
        <div class="label-row">
          ...
          <span class="char-counter" id="descCounter">
        ...
        <div class="invalid-feedback">모임 소개를 입력해주세요. (최대 1000자)</div>
      </div>
...
<script>
  ...
  // ----- 글자수 카운터 & 유효성 -----
  function bindCounter(inputEl, counterEl){
    const max = parseInt(inputEl.getAttribute('data-max'), 10);
    const currentEl = counterEl.querySelector('.current');

    function update(){
      // 강제 잘라내기 방지 대비, 초과 입력 시 자르기
      ...
      const atMax = (len >= max);
      counterEl.classList.toggle('is-max', atMax);
      inputEl.classList.toggle('is-max', atMax);

      // 사용자 정의 검증 메시지
      if (!len){
        inputEl.setCustomValidity('이 필드는 필수입니다.');
      } else if (len > max){
        inputEl.setCustomValidity(`최대 ${max}자까지 입력 가능합니다.`);
      } else {
  ...
  bindCounter(titleInput, titleCounter);
  bindCounter(descInput, descCounter);
  ...
  (function () {
    ...
    Array.from(forms).forEach(function (form) {
      form.addEventListener('submit', function (event) {
        // 길이 다시 확인
        if (titleInput.value.length === 0 || descInput.value.length === 0) {
          // 필수 입력 미충족
        }
